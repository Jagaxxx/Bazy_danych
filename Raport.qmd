...
Łącznie zorganizowana liczba wylotów wynosi `r laczna_liczba_wycieczek`. Najpopularniejsza wycieczka pod względem liczby wylotów to: `r najpopularniejsza[1]`, z liczbą wylotów `r najpopularniejsza[2]`, natomiast zyskująca dopiero popularność okazała się: `r najmniej_popularna[1]`, z liczbą wylotów `r najmniej_popularna[2]`.
...
Łącznie firma obsłużyła `r laczna_liczba_klientow` klientów. Najpopularniejsza wycieczka pod względem liczby klientów to: `r najwiecej_klientow[1]`, z liczbą klientów `r najwiecej_klientow[3]`, natomiast zyskująca dopiero popularność okazała się: `r najmniej_klientow[1]`, gdzie zdecydowało się na nią tyle osób: `r najmniej_klientow[3]`.
...
if (best_year > worse_year){
  if (bm_by_number_of_clients > bm_wy_number_of_clients){
    text <- paste("W roku", best_year, "miesiąc, w którym najwięcej klientów skorzystało z usługi to ", months[best_month_best_year], ". Dla porównania w roku", worse_year, "najbardziej zyskownym miesiącem był", months[best_month_worse_year], ". Sugeruje to, że z roku na rok do firmy przybywa coraz więcej klientów.")
  } else {
  ...
  if (bm_by_number_of_clients < bm_wy_number_of_clients){
    text <- paste("W roku ", best_year, " miesiąc, w którym najwięcej klientów skorzystało z usługi to ", months[best_month_best_year], ". Dla porównania w roku ", worse_year, " najbardziej zyskownym miesiącem był ", months[best_month_worse_year], ". Sugeruje to, że z roku na rok liczba klientów naszej firmy maleje.")
  } else {
...
ggplot(srednie_oceny, aes(x = reorder(trip_name, -srednia_ocena), y = srednia_ocena, fill = force_side)) +
  geom_col() +
  geom_text(aes(label = round(srednia_ocena, 2)), vjust = -0.5, size = 5) +
  scale_fill_manual(values = c("jasna" = "lightblue", "ciemna" = "pink")) +
  labs(
...
Najlepiej ocenianą wycieczką jest `r najlepsza_wycieczka[1]`, z oceną `r najlepsza_wycieczka[3]`. Najmniej do gustu przypadła za to wycieczka `r najgorsza_wycieczka[1]`, gdzie ocena wyniosła `r najgorsza_wycieczka[3]`.
