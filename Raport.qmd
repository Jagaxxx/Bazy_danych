...
if (nrow(df3_1) >= 3) {
  str_4_1 <- paste0(
    "Analizując powracających klientów, zauważyliśmy, że osoby, które zdecydowały się na więcej niż jeden wyjazd, najczęściej wybierały podczas pierwszej podróży następujące wycieczki: ",
    df3_1$nazwa_wycieczki[1], ", ", df3_1$nazwa_wycieczki[2], ", ", df3_1$nazwa_wycieczki[3], 
    ". Najwięcej klientów, którzy wrócili ponownie stanowili klienci, którzy za pierwszym razem wybrali wycieczkę: ", 
    most_repeated_trip, " (łącznie ", df3_1_1$liczba_klientów, " klientów). Może to oznaczać, że zwiedzającym spodobał się klimat wycieczki, a w planie firmy na najbliższy czas należy uwzględnić organizację większej liczby wycieczek tego typu bądź wprowadzić do oferty więcej rodzajów wyjazdów w podobnym klimacie."
  )
} else if (nrow(df3_1) == 2) {
  str_4_1 <- paste0(
    "Analizując powracających klientów, zauważyliśmy, że osoby, które zdecydowały się na więcej niż jeden wyjazd, najczęściej wybierały podczas pierwszej podróży następujące wycieczki: ", 
    df3_1$nazwa_wycieczki[1], ", ", df3_1$nazwa_wycieczki[2], 
    ". Najwięcej klientów, którzy wrócili ponownie stanowili klienci, którzy za pierwszym razem wybrali wycieczkę: ", 
    most_repeated_trip, " (łącznie ", df3_1_1$liczba_klientów, " klientów). Może to oznaczać, że zwiedzającym spodobał się klimat wycieczki, a w planie firmy na najbliższy czas należy uwzględnić organizację większej liczby wycieczek tego typu bądź wprowadzić do oferty więcej rodzajów wyjazdów w podobnym klimacie."
  )
} else if (nrow(df3_1) == 1) {
  str_4_1 <- paste0(
    "Analizując powracających klientów, zauważyliśmy, że osoby, które zdecydowały się na więcej niż jeden wyjazd, wybierały podczas pierwszej podróży wycieczkę: ", 
    df3_1$nazwa_wycieczki[1], 
    ". Wybrało ją łącznie ", df3_1_1$liczba_klientów, " klientów. Może to oznaczać, że zwiedzającym spodobał się klimat wycieczki, a w planie firmy na najbliższy czas należy uwzględnić organizację większej liczby wycieczek tego typu bądź wprowadzić do oferty więcej rodzajów wyjazdów w podobnym klimacie."
  )
} else {
  str_4_1 <- paste0(
    "Analizując wynik zapytania, można zauważyć, że żaden klient nie powrócił na kolejną wycieczkę po pierwszym wyjeździe z firmą. Oznacza to, że oferta wycieczek może być zupełnie nietrafiona. W tym przypadku należy przyjrzeć się dotychczasowym propozycjom wycieczek i zastanowić się nad ich zastąpieniem przez nowe propozycje, a następnie po pewnym czasie przeprowadzić ponowną analizę stopnia zainteresowania klientów."
  )
}
```
...
ggplot(df, aes(x = rodzaj_wypadku, y = liczba_wypadkow)) + 
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        plot.margin = margin(t = 10, r = 10, b = 40, l = 20) )
