...
ggplot(dane_long, aes(x = planet_name, y = wartosc, fill = typ)) +
  ...
  geom_text(aes(label = round(wartosc)), 
...
# Wynik
cat("Suma kosztów organizacji lotów:", round(suma_koszt_organizacji), "\n")
cat("Suma kosztów utrzymania klientów:", round(suma_koszt_utrzymania), "\n")
cat("Suma wypłat dla pracowników:", round(suma_wyplaty), "\n")
cat("Całkowity koszt:", round(suma_koszt_calkowity), "\n\n")

cat("Całkowity przychód:", round(suma_przychod), "\n")
cat("Zysk firmy:", round(zysk_firmy), "\n")
...
ggplot(df_wykres, aes(x = Rodzaj, y = Kwota, fill = Rodzaj)) +
  geom_col(width = 0.6) +
  geom_text(aes(label = round(Kwota)), vjust = -0.5, size = 5) +
  scale_fill_manual(values = c("Koszty" = "red", "Przychód" = "blue", "Zysk" = "green")) +
  labs(
